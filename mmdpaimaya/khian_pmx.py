...
def khiankhuen(chue_tem_file='', khanat_ok=0.125, chai_kraduk=1, chai_bs=1, chai_watsadu=1, lok_tex=1, mod_sph=1,
               tex_sph=''):
    t0 = time.time()
    chue_path_file = os.path.dirname(chue_tem_file)
    shoki = os.path.join(os.path.dirname(__file__), u'sample.pmx')
    ...
    if (chai_kraduk):
        ...
        for k in mc.ls(typ='joint', l=1):
            ...
            for chue_nod_skin in list_chue_nod_skin:
                if (chue_nod_skin and k in (pm.PyNode(x).fullPath() for x in
                                            mc.skinCluster(chue_nod_skin, q=1, inf=1))):
                    break
            # ถ้าไม่มีเลยก็ข้ามไป
            else:
                continue
            ...
            for chue_nod_kho in list_cp:
                if ('name_jp' in mc.listAttr(chue_nod_kho, ud = True)):
                    name_jp = mc.getAttr(chue_nod_kho + '.name_jp')
                else:
                    name_jp = chue_nod_kho
                name_en = chue_nod_kho.split('|')[-1];
                kw = {'name': name_jp, 'english_name': name_en, 'layer': 0, 'flag': 2074}
                ...
                if (jo == (0, 0, 0)):
                    ...
                    mm *= 1
                    if (mm):
                        mm *= om.MEulerRotation(math.radians(jo[0]), math.radians(jo[1]),
    ...
    for chue_nod_poly in list_chue_nod_poly:
        ...
        if (chai_bs):
            ...
            for i, chue_nod_bs in enumerate(list_chue_nod_bs_ni):
                ...
                if (mc.objExists(chue_nod_bs + '.name_jp')):
                    try:
                        dic_namae = dict(
                            namae.split(u'๑', 1) for namae in mc.getAttr(chue_nod_bs + '.name_jp').split(u'๐'))
                    except:
                        dic_namae = {}
                else:
                    dic_namae = {}
                ...
                for i, bs in enumerate(mc.listAttr(chue_nod_bs + '.w', m=1)):
                    ...
                    if (bs in dic_namae):
                        name_jp = dic_namae[bs].split(u'๑')
                        try:
                            list_chue_bs.append((name_jp[0], bs, int(name_jp[1])))
                        except:
                            list_chue_bs.append((name_jp[0], bs, 4))
            ...
            for j in range(chut_to_na):
                ...
                if (str_chut in dic_str_chut):
                    ...
                    for i_bs in range(n_bs):
                        ...
                        po = common.Vector3((p1[0] - p[0]) * khanat_ok, (p1[1] - p[1]) * khanat_ok,
                                            (p[2] - p1[2]) * khanat_ok)
        ...
        if (chai_bs):
            for i_mor, (chue_bs, luean_bs) in enumerate(zip(list_chue_bs, list_luean_bs)):
                model_mor_ap(pmx.Morph(name=chue_bs[0], english_name=chue_bs[1], panel=chue_bs[2], morph_type=1,
                                       offsets=luean_bs))
    ...
    if (chai_watsadu):
        ...
        for mat in mc.ls(mat=1):
            ...
            if (n_chut_to_mat == 0):
                continue
            if (mc.objExists(mat + '.name_jp')):
                name_jp = mc.getAttr(mat + '.name_jp')
            else:
                name_jp = mat
            # ค่าองค์ประกอบต่างๆของวัสดุ เก็บไว้ในดิกก่อน

            kw = {'name': name_jp, 'english_name': mat,
            ...
            try:
                ...
                if (nodetype not in ['aiStandard', 'aiStandardSurface']):
                    ...
                    if (nodetype == 'blinn'):
                        ...
                        kw['specular_color'] = common.RGB(
                            *[min(max(s, 0.), 1.) for s in mc.getAttr(mat + '.specularColor')[0]])
                # กรณี aiStandard
                else:
                    ...
                    if (nodetype != 'aiStandardSurface'):
                        kw['specular_color'] = common.RGB(
                            *[min(max(s, 0.), 1.) for s in mc.getAttr(mat + '.KsColor')[0]])
                    else:
                        kw['specular_color'] = common.RGB(
                            *[min(max(s, 0.), 1.) for s in mc.getAttr(mat + '.specularColor')[0]])
